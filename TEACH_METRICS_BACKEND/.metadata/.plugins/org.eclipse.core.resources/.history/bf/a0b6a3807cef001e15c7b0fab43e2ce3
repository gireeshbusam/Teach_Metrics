...
public class RegistrationServiceImpl implements RegistrationService {
    ...
    public HashMap<String, Object> submitProfile(profileModel pm) {
    	...
    	ValidationUtil valid = new ValidationUtil();
    	isValidFname = valid.isValidInput(pm.getfName());
    	isValidLname = valid.isValidInput(pm.getlName());
    	isValidContact = valid.isValidInput(pm.getContact());
    	isValidAddress = valid.isValidInput(pm.getAddress());
    	isValidPin = valid.isValidInput(pm.getPostalCode());
    	isValidExp = valid.isValidInput(pm.getExp());
    	
    	if(!isValidFname || !isValidLname || !isValidContact || !isValidAddress || !isValidPin || !isValidExp) {
    		response.put("responseMessage", "User Profile Submission Failed.");
			response.put("responseCode", 5555);
			return response;
    	}
